...
export default function Gallery(){
 ...
 return <><section className="gallery-band" aria-label="Galeria gabinetu">{photos.slice(0,3).map((src,i)=><button key={src} className={i===1?'wide':''} onClick={()=>setActive(i)} aria-label={`Otwórz zdjęcie ${i+1}`}><img src={src} alt="Wnętrze Ptak Stomatologia" loading="lazy" decoding="async" width="1620" height="1080"/>{i===1&&<span><Images/> Zobacz całą galerię</span>}</button>)}</section>
 {active!==null&&<div className="lightbox" role="dialog" aria-modal="true" aria-label="Galeria zdjęć" onClick={()=>setActive(null)}><button className="lightbox-close" onClick={()=>setActive(null)} aria-label="Zamknij"><X/></button><button className="lightbox-prev" onClick={e=>{e.stopPropagation();move(-1)}} aria-label="Poprzednie zdjęcie"><ArrowLeft/></button><div className="lightbox-stage" onClick={e=>e.stopPropagation()}><img src={photos[active]} alt={`Galeria Ptak Stomatologia — zdjęcie ${active+1}`} decoding="async"/><span>{active+1} / {photos.length}</span><div className="thumb-strip">{photos.map((src,i)=><button className={i===active?'active':''} key={src} onClick={()=>setActive(i)}><img src={src} alt="" loading="lazy" decoding="async"/></button>)}</div></div><button className="lightbox-next" onClick={e=>{e.stopPropagation();move(1)}} aria-label="Następne zdjęcie"><ArrowRight/></button></div>}</>;
}
